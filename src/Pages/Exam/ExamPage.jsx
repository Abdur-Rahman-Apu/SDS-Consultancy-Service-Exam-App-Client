...
const ExamPage = () => {
  ...
  const handleSubmit = (event) => {
    ...
    // Doing something with the userAnswers, e.g., send to the server
  ...
  return (
    <div className={`max-w-3xl mx-auto pb-24 font-Roboto`}>
        ...
        return (
          <div key={question.question} className="mt-24">
            <div className="my-10 bg-gray-100 shadow-xl px-10 py-5 rounded-lg">
              ...
              <h3 className="my-5 font-extrabold font-IBM sm:text-xl">
                {question}
              </h3>
      ...
      {/* Time Over Exam Screen Close */}
      {TimeFinished ? (
        <div className="fixed inset-0 flex justify-center transition-opacity duration-900 items-center h-full w-full bg-black bg-opacity-70">
          <div className="flex flex-col gap-4">
            <h1 className="text-4xl text-white">Exam Time Over!</h1>

            <p className="mt-10 text-xl text-green-500 space-x-10">
              <Link className="underline link-success" to="/">
                Home
              </Link>
              <Link className="underline link-success" to="/python/exam/result">
                Show Your Result
              </Link>
            </p>
          </div>
